...
<p style="color:green">
3) Use an external approach to control internal program shutdown
</p>
...
#### 1. Download the "spark_streaming_graceful_shutdown" application.
...
#### 2. Build the `spark_streaming_graceful_shutdown` application.
...
a) Ensure that you have the necessary permissions to run the script. If you encounter any permission issues, contact your system administrator or check the file permissions to ensure that you have execute permissions.

b) Update the BOOTSTRAP_SERVERS value in the script. The BOOTSTRAP_SERVERS variable should be set to the list of Kafka broker endpoints in the format <host1:port1>,<host2:port2>,.... Replace <host1:port1>,<host2:port2>,... with the actual Kafka broker endpoints for your cluster. For example:
